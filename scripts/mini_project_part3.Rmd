...
does not. The difference could be due to collinearity between `lot size ratio to neighbors` and other values, which leads to unstable p-value estimates and
could explain some of the difference in p-value. Another contributing factor to
the change could be the size of the test set, but because we performed feature
selection on the training set before fitting a model, problems with
post-selection inference seem to be the most likely cause of the difference.
