...
The confidence intervals are very similar for all variables except whether
a house is by the waterfront. For all other variable, the bootstrapped
...
on the assumption that errors are normally distributed, it makes sense that the
confidence interval for a binary variable with relatively few observations of
one class (only 85 homes out of the 12103 in the training set are waterfront
homes) that the bootstrapped confidence interval would be larger. For all other
variables, there is far more variety in the observations, so the confidence
intervals from both regression and bootstrapping are very small.
...
We performed feature selection using our training data before before fitting a
model. The decisions to uses select features that were correlated with the
training data means the p-values of our model on the training data are
downwardly biased. Looking at the linear model fit on the test test would yield
less biased results for p-values, but results in p-values with higher variance
because the test set is much smaller than the training set. When we look at 
p-values on the training set, we find that all variables have a p-value less
than 0.05, but on the test set `lot size ratio to neighbors` (i.e. `sqft_lot15`)
does not.
...
collinear, which means that the estimates for coefficients on each of these
variables are unstable as are the p-values. This collinearity could also explain
why `sqft_lot15` was significant when we fit the model to the training set
but when we fit it to the test set. 
...
5% of null hypotheses even when they are correct. Using the Bonferonni
correction, we obtain the following p-values:
```{r}
data.frame(summary(lm.train.fit)$coefficients) %>%
  bind_cols(data_frame(variable = names(lm.train.fit$coefficients))) %>%
  transmute(variable = variable,
            Correct.P =Pr...t../8,
            Less.Than.0.05 = Correct.P < 0.05)
```
Even with the Bonferonni correction, all p-values are significant.


##F: Causal Relationships

For the variables waterfont, grade, sqft_living,
and median_household_income, the coefficients are in the same direction we would
have expected before looking at the results of model fitting. 
Because sqft_living, sqft_living15, and sqft_lot15 are collinear, we are not
sure which (if any) actually causes the price of a home to change, although
sqft_living has the lowest p-value and highest coffiecient, matching what we
would have predicted.
For median household income, the regression coefficent shows that home prices
...
Waterfont and grade are far less correlated with other variables, extremely low
p-values, and have coefficients similar to what we would have predicted before
modeling: waterfront homes are much more expensive, and homes in better
condition are more expensive.
...
year -- from May 2014 to May 2015 so the model likely would not generalize well
to home prices outside that time period.
Because our features include some collinearity, the coefficients on some
features, particulary the two about the surrounding area, might not fully reflect
the relationship between the home price and the feature. 
...
do differently. While we think the regression problem of predicting house prices
is interesting, predicting whether a house will have a price above or below
the median price is less interesting because the distinction is not particularly
meaningful for the many house prices close to the median price so we would
choose a different metric to predict in a classification problem.
Using variable selection also makes our inferential claims weaker, as does
collinearity, so if we had only focused on inference and not prediction we
would have done more to reduce collinearity in our features, for example
converting sqft_lot15 and sqft_living15 to
