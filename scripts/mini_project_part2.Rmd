...
```{r}
# Read in data
zipcode_data <- read.csv(zipcode_file)
df <- read_csv(file_in) %>%
  dmap_at("date", ~ymd(.x)) %>%
  left_join(select(read.csv(zipcode_file), zipcode, median_household_income))
```
...
We added median income by zipcode as a feature. The income data comes
from the US Census Bureau's American Community Survey (ACS), a continual
survey used to help allocate federal funding.
